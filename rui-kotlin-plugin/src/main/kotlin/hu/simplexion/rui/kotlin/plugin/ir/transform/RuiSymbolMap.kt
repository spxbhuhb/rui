 ...
 */
package hu.simplexion.rui.kotlin.plugin.ir.transform

import hu.simplexion.rui.kotlin.plugin.ir.*
import hu.simplexion.rui.kotlin.plugin.ir.diagnostics.ErrorsRui.RUI_IR_INVALID_EXTERNAL_CLASS
import hu.simplexion.rui.kotlin.plugin.ir.util.RuiCompilationException
import hu.simplexion.rui.kotlin.plugin.ir.util.isSynthetic
...
class RuiSymbolMap(
    ...
    private val classSymbolMaps = mutableMapOf<FqName, hu.simplexion.rui.kotlin.plugin.ir.transform.RuiClassSymbols>()

    fun getSymbolMap(fqName: FqName): hu.simplexion.rui.kotlin.plugin.ir.transform.RuiClassSymbols =
        classSymbolMaps[fqName] ?: loadClass(fqName)

    private fun loadClass(fqName: FqName): hu.simplexion.rui.kotlin.plugin.ir.transform.RuiClassSymbols {
        val irClass =
        ...
        return hu.simplexion.rui.kotlin.plugin.ir.transform.RuiClassSymbols(irClass).also {
...
class RuiClassSymbols(
    ...
    private val stateVariables = mutableListOf<hu.simplexion.rui.kotlin.plugin.ir.transform.RuiStateVariableSymbol>()
    ...
    private fun property(indices: List<String>, it: IrProperty) {
        ...
        if (index >= 0) {
            stateVariables += hu.simplexion.rui.kotlin.plugin.ir.transform.RuiStateVariableSymbol(index, it)
            return
        }
    }
    ...
    fun getStateVariable(index: Int): hu.simplexion.rui.kotlin.plugin.ir.transform.RuiStateVariableSymbol =
